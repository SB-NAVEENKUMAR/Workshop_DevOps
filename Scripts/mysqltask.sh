...
# Enable binlog and server-id for master if not already present
if ! grep -q "server-id = 1" "$MASTER_CNF"; then
    sudo sed -i '/\[mysqld\]/a server-id = 1\nlog-bin = mysql-bin' "$MASTER_CNF"
    sudo systemctl restart mysql
fi

echo "=== Step 2: Setting Password & Creating Replication User on Master ==="
sudo mysql -u root -e "
ALTER USER 'root'@'localhost' IDENTIFIED VIA mysql_native_password USING PASSWORD('master');
CREATE USER IF NOT EXISTS 'repl'@'%' IDENTIFIED BY 'master';
GRANT REPLICATION SLAVE ON *.* TO 'repl'@'%';
FLUSH PRIVILEGES;
"

# Retrieve Master binary log file and position
MASTER_STATUS=$(sudo mysql --protocol=tcp --host=127.0.0.1 --port=3306 -u root -p'master' -e "SHOW MASTER STATUS\G")
...
echo "=== Step 5: Setting Password & Connecting Slave to Master ==="
sudo mysql --protocol=tcp --host=127.0.0.1 --port=3307 -u root -e "
ALTER USER 'root'@'localhost' IDENTIFIED VIA mysql_native_password USING PASSWORD('master');
FLUSH PRIVILEGES;
"

sudo mysql --protocol=tcp --host=127.0.0.1 --port=3307 -u root -p'master' -e "
STOP SLAVE;
CHANGE MASTER TO
...
sudo mysql --protocol=tcp --host=127.0.0.1 --port=3307 -u root -p'master' -e "SHOW SLAVE STATUS\G"
EOF

chmod +x setup_master_slave.sh
sudo ./setup_master_slave.sh
